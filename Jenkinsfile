pipeline {
  ...
  stages {
    stage('Build') {
      ...
      steps {
        slackSend color: 'warning', channel: '#dev', message: "Android app build ${env.BUILD_NUMBER} on branch ${BRANCH_NAME} is awaiting Release Notes input"
        script {
    ...
    stage('Decide release to beta') {
      ...
      steps {
        ...
        slackSend color: 'warning', channel: '#dev', message: "Android app build ${env.BUILD_NUMBER} on branch ${BRANCH_NAME} is awaiting user input for release to Beta"
        script {
    ...
    stage('Decide release to production') {
      ...
      steps {
        ...
        slackSend color: 'warning', channel: '#dev', message: "Android app build ${env.BUILD_NUMBER} on branch ${BRANCH_NAME} is awaiting user input for release to Production"
        script {
